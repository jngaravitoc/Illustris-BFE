"""Helpers for reproducing the notebook basis construction used in cell 9."""
...
def load_basis_fit() -> tuple[np.ndarray, np.ndarray]:
    ...
    fit_path = DATA_DIR / "dimer_density_profile_fit.txt"
    return np.loadtxt(fit_path)


def build_halo_21537_basis(nmax: int = 16, lmax: int = 8):
...
def load_test_halo_21537_basis(nmax: int = 16, lmax: int = 8):
    """Compute the halo 21537 basis using the same inputs as notebook cell 9.
    ...
    basis_params = {
        ...
        "lmax": int(lmax),
        "nmax": int(nmax),
        "cachename": str(TMP_BASIS_DIR / f"halo_21537_basis_cache_{nmax:02d}_{lmax:02d}.txt"),
        "modelname": str(TMP_BASIS_DIR / "halo_21537_model.txt"),
        "basis_id": "sphereSL",
    }
    ...
    basis = compute_basis(
        ...
        rho_norm_fit,
        str(TMP_BASIS_DIR),
        f"halo_21537_basis_config_{nmax:02d}_{lmax:02d}.yaml",
    )
    return basis


def test_basis_density_matches_reference() -> None:
    basis_obj = load_test_halo_21537_basis()
    basis_tests = build_halo_21537_basis()
