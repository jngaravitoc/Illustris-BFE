...
if __name__ == "__main__":
    ...
    OUTPUT_DIR   = Path('/n/nyx3/garavito/projects/Illustris-BFE/data/mwm31')
    ...
    for subhalo_id in subhalo_ids:
        output_file = OUTPUT_DIR / f'galaxies_tng50-3-dark_{subhalo_id}_099.hdf5'  # fixed: per-subhalo, inside loop

        if not output_file.exists():
            print(f'[SKIP] File not found: {output_file}')
            continue
        ...
        kde_mean = np.mean(np.asarray(kde_rho_3d[:, :, :], dtype=float), axis=2)
        bfe_mean = np.mean(np.asarray(bfe_rho_3d[:, :, :], dtype=float), axis=2)

        kde_positive = kde_mean[kde_mean > 0.0]
        kde_floor = max(np.nanmin(kde_positive), 1.0e-12) if kde_positive.size else 1.0e-12

        bfe_abs      = np.abs(bfe_mean)
        bfe_positive = bfe_abs[bfe_abs > 0.0]
        bfe_floor    = max(np.nanmin(bfe_positive), 1.0e-12) if bfe_positive.size else 1.0e-12

        kde_log = np.log10(np.clip(kde_mean, kde_floor, None))
        bfe_log = np.log10(np.clip(bfe_mean, bfe_floor, None))

        fig, axes = plt.subplots(1, 2, figsize=(12, 5), constrained_layout=True)

        kde_plot = axes[0].contourf(x_grid, y_grid, kde_log, levels=40, cmap='twilight', vmin=-4, vmax=1.6)
        axes[0].set_title(f'KDE density mid-plane — halo {subhalo_id}')
        axes[0].set_xlabel('x')
        axes[0].set_ylabel('y')
        axes[0].set_aspect('equal')
        fig.colorbar(kde_plot, ax=axes[0], label=r'$\log_{10}\,\rho_{\rm KDE}$')

        bfe_plot = axes[1].contourf(x_grid, y_grid, bfe_log, levels=40, cmap='twilight', vmin=-4, vmax=1.6)
        axes[1].set_title(f'BFE density mid-plane — halo {subhalo_id}')
        axes[1].set_xlabel('x')
        axes[1].set_ylabel('y')
        axes[1].set_aspect('equal')
        fig.colorbar(bfe_plot, ax=axes[1], label=r'$\log_{10}\,|\rho_{\rm BFE}|$')

        plt.savefig(OUTPUT_DIR / f'contour_fields_{subhalo_id}.png', dpi=150)  # fixed: unique filename per halo
        plt.close(fig)  # fixed: prevents memory leak over 198 iterations
